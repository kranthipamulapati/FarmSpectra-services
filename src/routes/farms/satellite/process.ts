...
processRouter.get(
    ...
    async ({ set, params }) => {
        ...
        try {
            ...
            if (processed === false) {
                const { farm_fk, tiff_path, visit_date } = tiffImage;
                ...
                for (let i = 0; i < redBand.length; i++) {
                    ...
                    osaviData[i] =
                        osaviDenom === 0 ? 0 : (nir - red) / osaviDenom;

                    const ndwiDenom = green + nir;
                    ndwiData[i] =
                        ndwiDenom === 0 ? 0 : (green - nir) / ndwiDenom;

                    const ndmiDenom = nir + swir;
                    ndmiData[i] =
                        ndmiDenom === 0 ? 0 : (nir - swir) / ndmiDenom;

                    const arviNumerator = nir - (2 * red - blue);
                    const arviDenom = nir + (2 * red + blue);
                    arviData[i] =
                        arviDenom === 0 ? 0 : arviNumerator / arviDenom;

                    const variDenom = green + red - blue;
                    variData[i] =
                        variDenom === 0 ? 0 : (green - red) / variDenom;
                    ...
                    sipiData[i] =
                        sipiDenom === 0 ? 0 : (nir - blue) / sipiDenom;
                    ...
                    mtvi2Data[i] =
                        sqrtTerm === 0 ? 0 : ntvi2Numerator / sqrtTerm;
