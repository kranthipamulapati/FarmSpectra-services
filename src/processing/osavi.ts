...
async function createOSAVIColorMap() {
    try {
        ...
        for (let i = 0; i < farms.length; i++) {
            ...
            for (let i = 0; i < redBand.length; i++) {
                const nir = nirBand[i];
                const red = redBand[i];
                osaviData[i] = (nir - red) / (nir + red + 0.16);
            }

            let minOSAVI = Infinity;
            let maxOSAVI = -Infinity;

            for (let i = 0; i < osaviData.length; i++) {
                if (osaviData[i] < minOSAVI) minOSAVI = osaviData[i];
                if (osaviData[i] > maxOSAVI) maxOSAVI = osaviData[i];
            }

            const rgbData = Buffer.alloc(width * height * 3);

            for (let i = 0; i < osaviData.length; i++) {
                const osavi = osaviData[i];
                let r, g, b;

                if (osavi < 0) {
                    const intensity = Math.max(0, 1 + osavi * 2);
                    r = Math.round(139 * intensity);
                    g = Math.round(69 * intensity);
                    b = Math.round(19 * intensity);
                } else if (osavi < 0.2) {
                    const ratio = osavi / 0.2;
                    r = Math.round(255 * (1 - ratio));
                    g = 255;
                    b = Math.round(50 * ratio);
                } else if (osavi < 0.4) {
                    const ratio = (osavi - 0.2) / 0.2;
                    r = 0;
                    g = 255;
                    b = Math.round(50 + 50 * ratio);
                } else if (osavi < 0.6) {
                    const ratio = (osavi - 0.4) / 0.2;
                    r = 0;
                    g = Math.round(255 * (1 - ratio * 0.5));
                    b = Math.round(100 * (1 - ratio));
                } else {
                    const ratio = Math.min(1, (osavi - 0.6) / 0.4);
                    r = 0;
            ...
            await sharp(rgbData, {
                ...
                .png()
                .toFile(
                    `./images/${farm_fk}/${date}/${collection_code}/osavi.png`
                );
        }
    } catch (error) {
