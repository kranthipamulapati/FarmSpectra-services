...
async function createVARIColorMap() {
    try {
        ...
        for (let i = 0; i < farms.length; i++) {
            ...
            for (let i = 0; i < redBand.length; i++) {
                ...
                variData[i] = (green - red) / (green + red - blue);
            }

            let minVARI = Infinity;
            let maxVARI = -Infinity;

            for (let i = 0; i < variData.length; i++) {
                if (variData[i] < minVARI) minVARI = variData[i];
                if (variData[i] > maxVARI) maxVARI = variData[i];
            }

            const rgbData = Buffer.alloc(width * height * 3);

            for (let i = 0; i < variData.length; i++) {
                const vari = variData[i];
                let r, g, b;

                if (vari < 0) {
                    const intensity = Math.max(0, 1 + vari * 2);
                    r = Math.round(139 * intensity);
                    g = Math.round(69 * intensity);
                    b = Math.round(19 * intensity);
                } else if (vari < 0.2) {
                    const ratio = vari / 0.2;
                    r = Math.round(255 * (1 - ratio));
                    g = 255;
                    b = Math.round(50 * ratio);
                } else if (vari < 0.4) {
                    const ratio = (vari - 0.2) / 0.2;
                    r = 0;
                    g = 255;
                    b = Math.round(50 + 50 * ratio);
                } else if (vari < 0.6) {
                    const ratio = (vari - 0.4) / 0.2;
                    r = 0;
                    g = Math.round(255 * (1 - ratio * 0.5));
                    b = Math.round(100 * (1 - ratio));
                } else {
                    const ratio = Math.min(1, (vari - 0.6) / 0.4);
                    r = 0;
            ...
            await sharp(rgbData, {
                ...
                .png()
                .toFile(
                    `./images/${farm_fk}/${date}/${collection_code}/vari.png`
                );
        }
    } catch (error) {
