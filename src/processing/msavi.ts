...
async function createMSAVIColorMap() {
    try {
        ...
        for (let i = 0; i < farms.length; i++) {
            ...
            for (let i = 0; i < redBand.length; i++) {
                ...
                msaviData[i] =
                    (2 * nir +
                        1 -
                        Math.sqrt((2 * nir + 1) ** 2 - 8 * (nir - red))) /
                    2;
            }

            let minMSAVI = Infinity;
            let maxMSAVI = -Infinity;

            for (let i = 0; i < msaviData.length; i++) {
                if (msaviData[i] < minMSAVI) minMSAVI = msaviData[i];
                if (msaviData[i] > maxMSAVI) maxMSAVI = msaviData[i];
            }

            const rgbData = Buffer.alloc(width * height * 3);

            for (let i = 0; i < msaviData.length; i++) {
                const msavi = msaviData[i];
                let r, g, b;

                if (msavi < 0) {
                    const intensity = Math.max(0, 1 + msavi * 2);
                    r = Math.round(139 * intensity);
                    g = Math.round(69 * intensity);
                    b = Math.round(19 * intensity);
                } else if (msavi < 0.2) {
                    const ratio = msavi / 0.2;
                    r = Math.round(255 * (1 - ratio));
                    g = 255;
                    b = Math.round(50 * ratio);
                } else if (msavi < 0.4) {
                    const ratio = (msavi - 0.2) / 0.2;
                    r = 0;
                    g = 255;
                    b = Math.round(50 + 50 * ratio);
                } else if (msavi < 0.6) {
                    const ratio = (msavi - 0.4) / 0.2;
                    r = 0;
                    g = Math.round(255 * (1 - ratio * 0.5));
                    b = Math.round(100 * (1 - ratio));
                } else {
                    const ratio = Math.min(1, (msavi - 0.6) / 0.4);
                    r = 0;
            ...
            await sharp(rgbData, {
                ...
                .png()
                .toFile(
                    `./images/${farm_fk}/${date}/${collection_code}/msavi.png`
                );
        }
    } catch (error) {
